...
/*
props for dev / artist
second function to erase branches
decrement steps until 0 ??
change window side ? right:50? conditional class?
 */
onUpdated(() => {
  if (props.artistHover) {
    branch()
  } else if (props.devHover) {
    branch()
  } else {
    branch(true)
  }
...
const branch = async (nothing?: boolean) => {
  if (nothing) {
    const canvas = el.value!
    const { ctx } = initCanvas(canvas, size.width /2 , size.height)
  ...
  const canvas = el.value!
  const { ctx } = initCanvas(canvas, size.width / 3 , size.height)
  const { width, height } = canvas
